/* ─────────────────────────────────────────────────────────────────────────────
   ClearTrace — brsr-p5.js
   BRSR Principle 5 — Human Rights data entry form.
   Follows brsr-p3.js exactly. Adds per-field BRSR Core badges and evidence
   widgets (E2, E3) identical to how P6 handles energy and GHG panels.
   ───────────────────────────────────────────────────────────────────────────── */
...
window.toggleLeadership = function(header) {
  const body = document.getElementById('p5-leadership-body');
  if (!body) return;
...
// ─────────────────────────────────────────────────────────────────────────────
// CALC FORMULA HELPERS
// Supports: pct(numeratorCol, denominatorCol) → (a/b*100).toFixed(1)%
// ─────────────────────────────────────────────────────────────────────────────
...
// Recalculate all calc cells in one specialist-table row from live DOM inputs.
function recalcSpecialistRow(fieldKey, rowKey) {
  ...
  for (const col of field.table_schema.columns) {
    ...
    const cellEl = document.getElementById(`p5-calc-${fieldKey}-${rowKey}-${col.key}`);
    if (!cellEl) continue;
...
// Extends brsr-p3.js renderSpecialistTable with calc and select column types.
// ─────────────────────────────────────────────────────────────────────────────
        ...
        if (em) {
          html += `<td class="p6-spec-td p3-calc-td">
            <span class="p3-calc-cell"
              id="p5-calc-${escAttr(field.key)}-${escAttr(row.key)}-${escAttr(col.key)}">
              ${display}
            </span>
          </td>`;
        } else {
...
// calc columns carry no data-spec-field attribute — automatically skipped
...
// ─────────────────────────────────────────────────────────────────────────────
document.getElementById('p5-panels').addEventListener('input', e => {
  if (!editMode) return;
  const inp       = e.target;
...
function renderPanels(em) {
  const container = document.getElementById('p5-panels');
  container.innerHTML = '';
  ...
  for (const cat of essentialCats) {
    ...
    // Mount per-field evidence widgets for BRSR Core fields
    for (const f of cat.fields) {
      if (f.brsr_core && f.field_ref && submissionId) {
        const evEl = panel.querySelector(`#ev-field-${f.key}`);
        if (evEl) mountEvidenceWidget(evEl, submissionId, f.field_ref, { readonly: _isLocked });
      }
    }
  }
  ...
  if (leadershipCats.length) {
    ...
    section.innerHTML = `
      ...
      <div class="p6-leadership-body p6-leadership-body--collapsed" id="p5-leadership-body"></div>`;
    container.appendChild(section);
    const body = section.querySelector('#p5-leadership-body');
...
function buildPanel(cat, em) {
  ...
  const catCoreBadge = cat.brsr_core
    ? `<span class="p6-core-badge">● BRSR Core — Assurance Required</span>`
    : '';

  const fieldRows = cat.fields.map(f => {
    const fieldCoreBadge = f.brsr_core
      ? `<span class="p6-core-badge">● BRSR Core — Assurance Required</span>`
      : '';
    const evidenceSlot = f.brsr_core
      ? `<div class="p6-evidence-slot" id="ev-field-${escAttr(f.key)}"></div>`
      : '';
    return `
      <div class="sa-field-row">
        <div class="sa-field-label">${escHtml(f.label)}${fieldCoreBadge}${f.unit ? `<span class="p6-unit-hint"> (${escHtml(f.unit)})</span>` : ''}</div>
...
async function saveAll() {
  ...
  for (const payload of saves) {
    const r = await api('PUT', `/api/brsr/p5/${submissionId}`, payload);
    if (!r || !r.ok) errors++;
  }
...
async function reloadP5() {
  const res = await api('GET', `/api/brsr/p5/${submissionId}`);
  if (!res || !res.ok) { showToast('Failed to load P5 data', 'error'); return; }
  const data = await res.json();
