/* ─────────────────────────────────────────────────────────────────────────────
   ClearTrace — brsr-p3.js
   BRSR Principle 3 — Employee Wellbeing data entry form.
   Follows brsr-p6.js exactly. Adds calc column support to renderSpecialistTable.
   ───────────────────────────────────────────────────────────────────────────── */
...
window.toggleLeadership = function(header) {
  const body = document.getElementById('p3-leadership-body');
  if (!body) return;
...
function recalcSpecialistRow(fieldKey, rowKey) {
  ...
  for (const col of field.table_schema.columns) {
    ...
    const cellEl = document.getElementById(`p3-calc-${fieldKey}-${rowKey}-${col.key}`);
    if (!cellEl) continue;
...
// Extends brsr-p6.js renderSpecialistTable with:
//   - 'calc' column type  (display-only, never saved)
//   - 'select' column type
// ─────────────────────────────────────────────────────────────────────────────
        ...
        if (em) {
          html += `<td class="p6-spec-td p3-calc-td">
            <span class="p3-calc-cell"
              id="p3-calc-${escAttr(field.key)}-${escAttr(row.key)}-${escAttr(col.key)}">
              ${display}
            </span>
          </td>`;
        } else {
...
// collect: calc columns carry no data-spec-field attribute, so they're automatically skipped
...
// ─────────────────────────────────────────────────────────────────────────────
// DYNAMIC TABLE (copied from brsr-p6.js)
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
document.getElementById('p3-panels').addEventListener('input', e => {
  if (!editMode) return;
  const inp       = e.target;
...
function renderPanels(em) {
  ...
  if (leadershipCats.length) {
    ...
    section.innerHTML = `
      ...
      <div class="p6-leadership-body p6-leadership-body--collapsed" id="p3-leadership-body"></div>`;
    container.appendChild(section);
    const body = section.querySelector('#p3-leadership-body');
...
async function saveAll() {
  ...
  for (const payload of saves) {
    const r = await api('PUT', `/api/brsr/p3/${submissionId}`, payload);
    if (!r || !r.ok) errors++;
  }
...
async function reloadP3() {
  const res = await api('GET', `/api/brsr/p3/${submissionId}`);
  if (!res || !res.ok) { showToast('Failed to load P3 data', 'error'); return; }
  const data = await res.json();
