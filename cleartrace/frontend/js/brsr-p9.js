/* ─────────────────────────────────────────────────────────────────────────────
   ClearTrace — brsr-p9.js
   BRSR Principle 9 — Consumer Responsibility data entry form.
   ───────────────────────────────────────────────────────────────────────────── */
...
window.toggleLeadership = function(header) {
  const body = document.getElementById('p9-leadership-body');
  if (!body) return;
...
function recalcSpecialistRow(fieldKey, rowKey) {
  ...
  for (const col of field.table_schema.columns) {
    ...
    const cellEl = document.getElementById(`p9-calc-${fieldKey}-${rowKey}-${col.key}`);
    if (!cellEl) continue;
        ...
        if (em) {
          html += `<td class="p6-spec-td p3-calc-td">
            <span class="p3-calc-cell"
              id="p9-calc-${escAttr(field.key)}-${escAttr(row.key)}-${escAttr(col.key)}">
              ${display}
            </span>
          </td>`;
        } else {
...
document.getElementById('p9-panels').addEventListener('input', e => {
  if (!editMode) return;
  const inp       = e.target;
...
function renderPanels(em) {
  const container = document.getElementById('p9-panels');
  container.innerHTML = '';
  ...
  if (leadershipCats.length) {
    ...
    section.innerHTML = `
      ...
      <div class="p6-leadership-body p6-leadership-body--collapsed" id="p9-leadership-body"></div>`;
    container.appendChild(section);
    const body = section.querySelector('#p9-leadership-body');
...
async function saveAll() {
  ...
  for (const payload of saves) {
    const r = await api('PUT', `/api/brsr/p9/${submissionId}`, payload);
    if (!r || !r.ok) errors++;
  }
...
async function reloadP9() {
  const res = await api('GET', `/api/brsr/p9/${submissionId}`);
  if (!res || !res.ok) { showToast('Failed to load P9 data', 'error'); return; }
  const data = await res.json();
