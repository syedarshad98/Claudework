...
const { GOVERNANCE_METRICS } = require('../lib/sg-metrics');
...
// ── GET /api/governance/metrics?period=YYYY-MM ────────────────────────────────
router.get('/metrics', async (req, res) => {
  ...
  try {
    ...
    res.json({
      period,
      categories: mergeMetrics(GOVERNANCE_METRICS, saved.rows, sparklineMap),
    });
  } catch (err) {
    console.error('GET /api/governance/metrics error:', err.message);
    res.status(500).json({ error: 'Failed to fetch governance metrics' });
  }
});

// ── POST /api/governance/metrics — upsert a single metric value ───────────────
router.post('/metrics', requireRole('admin', 'editor'), async (req, res) => {
  ...
  const allKeys = Object.values(GOVERNANCE_METRICS).flatMap(c => c.metrics.map(m => m.key));
  ...
  try {
    ...
    console.error('POST /api/governance/metrics error:', err.message);
    res.status(500).json({ error: 'Failed to save metric' });
  }
});

// ── GET /api/governance/history/:metric_key — last 8 periods ──────────────────
router.get('/history/:metric_key', async (req, res) => {
  ...
  try {
    ...
    console.error('GET /api/governance/history error:', err.message);
    res.status(500).json({ error: 'Failed to fetch metric history' });
  }
});

// ── GET /api/governance/summary — most recent period with any data ─────────────
router.get('/summary', async (req, res) => {
  await ensureMigrated();
  try {
    // Find the most recent period that has data
    const periodRes = await db.query(
      `SELECT period FROM governance_metrics
        WHERE company_id = $1
    ...
    console.error('GET /api/governance/summary error:', err.message);
