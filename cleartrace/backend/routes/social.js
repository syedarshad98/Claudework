...
const { SOCIAL_METRICS } = require('../lib/sg-metrics');
...
async function ensureMigrated() {
  ...
  const sql  = fs.readFileSync(path.join(__dirname, '../db/sg_migration.sql'), 'utf8');
  await db.query(sql);
  migrated = true;
}
...
// ── GET /api/social/metrics?period=YYYY-MM ────────────────────────────────────
router.get('/metrics', async (req, res) => {
  ...
  try {
    ...
    res.json({
      period,
      categories: mergeMetrics(SOCIAL_METRICS, saved.rows, sparklineMap),
    });
  } catch (err) {
    console.error('GET /api/social/metrics error:', err.message);
    res.status(500).json({ error: 'Failed to fetch social metrics' });
  }
});

// ── POST /api/social/metrics — upsert a single metric value ──────────────────
router.post('/metrics', requireRole('admin', 'editor'), async (req, res) => {
  ...
  const allKeys = Object.values(SOCIAL_METRICS).flatMap(c => c.metrics.map(m => m.key));
  ...
  try {
    ...
    console.error('POST /api/social/metrics error:', err.message);
    res.status(500).json({ error: 'Failed to save metric' });
  }
});

// ── GET /api/social/history/:metric_key — last 8 periods ─────────────────────
router.get('/history/:metric_key', async (req, res) => {
  ...
  try {
    ...
    console.error('GET /api/social/history error:', err.message);
    res.status(500).json({ error: 'Failed to fetch metric history' });
  }
});

// ── GET /api/social/summary — most recent period with any data ────────────────
router.get('/summary', async (req, res) => {
  await ensureMigrated();
  try {
    // Find the most recent period that has data
    const periodRes = await db.query(
      `SELECT period FROM social_metrics
        WHERE company_id = $1
    ...
    console.error('GET /api/social/summary error:', err.message);
    res.status(500).json({ error: 'Failed to fetch social summary' });
  }
});
