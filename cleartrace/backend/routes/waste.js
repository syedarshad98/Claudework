...
const { WASTE_METRICS } = require('../lib/env-metrics');
...
// ── GET /api/waste/metrics?period=YYYY-MM ─────────────────────────────────────
router.get('/metrics', async (req, res) => {
  ...
  try {
    ...
    res.json({
      period,
      categories: mergeMetrics(WASTE_METRICS, saved.rows, sparklineMap),
    });
  } catch (err) {
    console.error('GET /api/waste/metrics error:', err.message);
    res.status(500).json({ error: 'Failed to fetch waste metrics' });
  }
});

// ── POST /api/waste/metrics — upsert a single metric value ───────────────────
router.post('/metrics', requireRole('admin', 'editor'), async (req, res) => {
  ...
  const allKeys = Object.values(WASTE_METRICS).flatMap(c => c.metrics.map(m => m.key));
  ...
  try {
    ...
    console.error('POST /api/waste/metrics error:', err.message);
    res.status(500).json({ error: 'Failed to save metric' });
  }
});

// ── GET /api/waste/history/:metric_key — last 8 periods ──────────────────────
router.get('/history/:metric_key', async (req, res) => {
  ...
  try {
    ...
    console.error('GET /api/waste/history error:', err.message);
    res.status(500).json({ error: 'Failed to fetch metric history' });
  }
});

// ── GET /api/waste/summary — most recent period with any data ─────────────────
router.get('/summary', async (req, res) => {
  await ensureMigrated();
  try {
    // Find the most recent period that has data
    const periodRes = await db.query(
      `SELECT period FROM waste_metrics
        WHERE company_id = $1
    ...
    console.error('GET /api/waste/summary error:', err.message);
